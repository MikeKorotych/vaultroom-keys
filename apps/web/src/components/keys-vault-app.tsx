  ...
  return (
    <main className="keysApp" data-sync={syncState} data-rail-compact={railCompact}>
      ...
      <section className="keysWorkspace">
        ...
        <div className="keysHero"><p className="keysKicker">LOCAL DECRYPTED VIEW</p><section className="keysHeroTitle"><h1>Secrets</h1><small>{filter === "all" ? "ALL ENVIRONMENTS" : filter.toUpperCase()}</small></section><div><span>{payload?.items.length ?? 0}<small>items</small></span><span>{payload?.items.filter((item) => item.environment === "production").length ?? 0}<small>production</small></span></div></div>
